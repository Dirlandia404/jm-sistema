...
if ($route === "user-store" && $_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        $registered = $authController->register($name, $email, $password);
    } catch (\PDOException $exception) {
        error_log($exception->getMessage());
        $registered = false;
    }
...
if ($route === "dashboard") {
    ...
    $serviceName = trim((string) ($_GET["service_name"] ?? ""));
    $userName = trim((string) ($_GET["user_name"] ?? ""));

    $status = trim((string) ($_GET["status"] ?? ""));

    $allowedStatuses = ["", "pendente", "finalizado"];

    if (!in_array($status, $allowedStatuses, true)) {
        $_SESSION["service_error"] = "Status informado é inválido.";
    ...
    if (
        ...
        $_SESSION["service_error"] = "Período informado é inválido.";
    ...
    $totalServices = $serviceModel->getTotalByUserId($userId);
    $pendingServices = $serviceModel->findLatestPendingByUserId($userId);
